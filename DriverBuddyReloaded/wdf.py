...
def populate_wdf():
    ...
    for segStartEa in idautils.Segments():
        # print(f"[FG] segStartEa={hex(segStartEa)}")
        mySegment = idaapi.getseg(segStartEa)
        # print("[FG] mySegment={}".format(mySegment))
        mySegmentName = idc.get_segm_name(segStartEa)
        # print(f"[FG] mySegmentName={mySegmentName}")
        segm = mySegment
        # print("[FG] segm={}".format(segm))
        if segm is None:
            continue
        # print(f"[FG] segm.start_ea={hex(segm.start_ea)} segm.end_ea={hex(segm.end_ea)}")
        if segm.start_ea != idc.BADADDR and segm.end_ea != idc.BADADDR:
            ...
            # print(f"[FG] idx={hex(idx)} idaapi.BADADDR={hex(idaapi.BADADDR)}")
